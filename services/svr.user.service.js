...
function addDomain(body) {
    var deferred = Q.defer();
    console.log("body = ", body);
    //==================CONSTANTS=================================================
    // AWS.Credentials({
    //     accessKeyId: 'AKIAIALZ6BYNWRSLXKLQ', secretAccessKey: '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'
    // });

    process.env.AWS_ACCESS_KEY_ID = "AKIAIALZ6BYNWRSLXKLQ"
    process.env.AWS_SECRET_ACCESS_KEY = "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"
    ...
    var route53 = new AWS.Route53({ apiVersions: "2013-04-01" });

    //===================================================================
    if (!body.domainName) {
        deferred.reject("Please enter name.")
    }

    if (!body.pointTo) {
        deferred.reject("Please enter targeted ip address.")
    }

    if (!body.comment) {
        deferred.reject("Please enter comment.")
    }

    // if (!body.hostedZoneId) {
    //     deferred.reject("Please enter hosted zone id.")
    // }
    ...
    var params = {
        ...
        HostedZoneId: "ZS7DII6EVG5SC"
    };
    console.log("params = ", params)
    route53.changeResourceRecordSets(params, function (err, data) {
        console.log("domain create res : ", data)
        console.log("domain create res error : ", err)
        if (err) deferred.reject(err, err.stack); // an error occurred
        else deferred.resolve({ message: "Registration Successful" });           // successful response
    });
